    ...
    <div class="title">
      ...
      <n-flex class="status">
        ...
        <n-text class="item">
          ...
          <n-number-animation :from="0" :to="allMusicSize" :precision="2" /> MB
        </n-text>
      </n-flex>
    </div>
...
import { isElectron, fuzzySearch, renderIcon } from "@/utils/helper";
...
const getMusicFolder = async (): Promise<string[]> => {
  if (isElectron) {
    ...
    // 在网页版中，我们只使用服务器上的 localmusic 文件夹
    return ["/localmusic"];
  }
};
...
const allMusicSize = computed<number>(() => {
  const total = localStore.localSongs.reduce((total, song) => (total += Number(song?.size) || 0), 0);
